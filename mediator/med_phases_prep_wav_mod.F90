...
!-----------------------------------------------------------------------------
  ...
  subroutine med_phases_prep_wav_accum(gcomp, rc)
    ...
    ! custom merges to wav
    if (trim(coupling_mode) == 'hafs_mom6') then
       call med_phases_prep_wav_custom_hafs_mom6(gcomp, rc)
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
    end if
  ...
  subroutine med_phases_prep_wav_custom_hafs_mom6(gcomp, rc)
    ...
    if (dbug_flag > 20) then
       if (maintask) then
          ...
          write(logunit,'(L1)')  first_time
       end if
    end if
    if (first_time) then
! Sa_u10m and Sa_v10m are from GFS
       call hafs_merge_forcing(is_local%wrap%FBExp(compwav),   'Sa_u10m',  &
            ...
            FBinB=is_local%wrap%FBImp(compdat,compwav), fnameB='Sd_u10m', wgtB=wgtp01, rc=rc)
       if (ChkErr(rc,__LINE__,u_FILE_u)) return

       call hafs_merge_forcing(is_local%wrap%FBExp(compwav),   'Sa_v10m',  &
            FBinA=is_local%wrap%FBImp(compdat,compwav), fnameA='Sd_v10m', wgtA=wgtp01, &
            FBinB=is_local%wrap%FBImp(compdat,compwav), fnameB='Sd_v10m', wgtB=wgtp01, rc=rc)
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
    else
       call hafs_merge_forcing(is_local%wrap%FBExp(compwav),   'Sa_u10m',  &
            FBinA=is_local%wrap%FBImp(compdat,compwav), fnameA='Sd_u10m', wgtA=wgtp01, &
       ...
       call hafs_merge_forcing(is_local%wrap%FBExp(compwav),   'Sa_v10m',  &
            FBinA=is_local%wrap%FBImp(compdat,compwav), fnameA='Sd_v10m', wgtA=wgtp01, &
            FBinB=is_local%wrap%FBImp(compatm,compwav), fnameB='Sa_v10m', wgtB=wgtp01, rc=rc)
       if (ChkErr(rc,__LINE__,u_FILE_u)) return
    end if
    first_time = .false.
